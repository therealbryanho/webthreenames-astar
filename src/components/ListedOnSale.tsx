...
export const ListedOnSale = () => {
  ...
  const loadMyListedDomains = async () => {
    if (account) {
      ...
      if (result) {
        ...
        const ids: any = [];
        trimmed.map((i: any) => ids.push(i.tokenId))
  ...
  const unlistDomain = async () => {
    ...
    try {
      ...
      const tx = await contract['removeMarketItem'](
        selectedDomain[0].itemId,
        CONTRACT_ADDRESS,
        selectedDomain[0].tokenId,
        {
          from: account,
          gasLimit: 3000000
        }
      ...
      if (receipt.status === 1) {
        ...
        loadMyListedDomains(); //reload the domains on market
                            ...
                            myListed.map((mint: any, key: any) => (
                              <div key={key} className="myMint">
                                ...
                                <div className='myMint-footer'>
                                  <button className='mint-button' onClick={() => { setSelectedDomain([mint, domainData[key]]); showModal() }}>Unlist</button>
                    ...
                    <Modal show={show} onHide={closeModal} centered size="lg">
                      ...
                      < Modal.Body >
                        <div className='modal-body'>
                          <p>Please confirm if you want to unlist your domain from the market</p>
